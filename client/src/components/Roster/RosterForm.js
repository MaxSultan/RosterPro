...
export default function RosterForm(props) {
  const [name, setName] = useState(
    props.selectedRosterName ? props.selectedRosterName : ""
  );
  const [year, setYear] = useState(
    props.selectedRosterYear ? props.selectedRosterYear : ""
  );

  const yearOptions = () => {
    var yearList = [];
    for (
      var i = new Date().getFullYear() - 11;
  ...
  return (
    <div className="roster-form">
      <form onSubmit={handleSubmit} className="verticalAlign form">
        ...
        />
        <label for="year" className="formLabel">
          Year:
        </label>
        <select
          name="year"
